...
public:
  ...
  void drawPoints(Graphics &g) {
    if (_pointPositions.empty() || _pointPositions[0].empty())
      return;

    for (size_t i = 0; i < _latentsNbr; i++) {
      if (_pointPositions[i].empty())
        continue;
      auto point = _pointPositions[i].back();
  ...
  void drawVizLines(Graphics &g) {
    if (_pointPositions.empty())
      return;

    size_t trajLength = _pointPositions[0].size();
    if (trajLength == 0)
      return;

    for (size_t i = 1; i < _latentsNbr; i++) {
      trajLength = std::min(trajLength, _pointPositions[i].size());
    }
    if (trajLength == 0)
      return;
  ...
  void timerCallback() {
    ...
    if (latent.ndimension() < 2)
      return;
    latent = latent[0];
    if (latent.ndimension() < 2 || latent.sizes()[0] <= 0 || latent.sizes()[1] <= 0)
      return;

    const size_t latentSizes = std::min((size_t)latent.sizes()[0], _latentsNbr);
    int latentTrajLength = (int)latent.sizes()[1];
    int64_t currentIdx =
  ...
  size_t getHoveredLatent(Point<int> pos) {
    ...
    if (res > 0)
      return ((size_t)std::floor(res)) % _latentsNbr;
    // Wrap negative values to [0, _latentsNbr).
    int wrapped = ((int)std::floor(res) % (int)_latentsNbr + (int)_latentsNbr) %
                  (int)_latentsNbr;
